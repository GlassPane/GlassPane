...
public class LayerCapeCustom extends LayerCape implements IHasSkin<EntityPlayer> {
    ...
    protected ResourceLocation getCapeTexture(AbstractClientPlayer player) {
        ResourceLocation skin = this.getSkin(player, player.world, player);
        return skin != null ? new ResourceLocation(GlassPane.MODID, "textures/entity/" + skin.getNamespace() + "/" + skin.getPath() + ".png") : player.getLocationCape();
    }
    ...
    @Override
    public void setSkin(EntityPlayer player, @Nullable ResourceLocation skin) {
        //NO-OP
    }

    @Nullable
    @Override
    public ResourceLocation getSkin(EntityPlayer player, @Nullable World world, @Nullable Entity entity) {
